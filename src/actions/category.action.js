...
const getAllCategory = () => {
    return async dispatch => {
        ...
        const res = await axios.get(`http://localhost:2000/api/user/getcategory`);
        console.log(res);
        if (res.status === 200) {
...
export const addCategory = (form) => {
    ...
    return async dispatch => {
        ...
        try {
            const res = await axios.post(`http://localhost:2000/api/user/addcat`, form);
            if (res.status === 201) {
                dispatch({
...
export const deleteCategories = (ids) => {
    return async dispatch => {
        ...
        const res = await axios.post(`http://localhost:2000/api/user/category/delete/:id`, {
            payload: {
                ids
            }
        });
